...
def menu():
    ...
    print("4. Prêmio 4")
    print("5. Prêmio 5")
    escolha = input("Escolha o número do prêmio para análise (1 a 5) ou 'q' para sair: ").strip().lower()
    if escolha == 'q':
        return None
    elif escolha.isdigit() and 1 <= int(escolha) <= 5:
        return int(escolha)
    else:
...
def main():
    ...
    while True:
        ...
        continuar = input("Deseja continuar buscando repetições? (S/N): ").strip().lower()
        if continuar != 's':
            break
